...
\begin{lemma} [Generalized hockey stick identity]
  ...
  \end{proof}
\end{lemma}
Applying the identity for binomial coefficients
$\binom{-k}{j} = (-1)^j \binom{j+k-1}{j}$, we obtain
\begin{proposition} [Ordinary sums of powers via backward differences]
  ...
  \end{align*}
\end{proposition}
For example, by setting $t=2$ and $m=1,2,3,4$,
we get formulas for sums of cubes
\begin{align*}
  \KnuthRFoldSum{1}{n}{1} &=
...
\begin{proposition} [Ordinary Stirling sums of powers in zero altered]
  ...
  For non-negative integers $n,m$
  \begin{align*}
    \KnuthRFoldSum{1}{n}{m} &= \sum_{j=0}^{m} \sum_{k=j}^{m} (-1)^{k-j} \binom{k-1}{j-1} \binom{j+n}{j+1}  \stirlingii{m}{k} k!
  \end{align*}
\end{proposition}
