...
\begin{lemma} [Backward differences in Eulerian numbers]
    ...
    \begin{align*}
        \Delta^{j} t^{m} = \sum_{k=0}^{m} \eulerian{m}{k} \binom{t+k-j}{m-j}
    \end{align*}
    \begin{proof}
        By Worpitzky identity $t^{m} = \sum_{k=0}^{m} \eulerian{m}{k} \binom{t+k}{m}$ and binomial recurrence $\binom{n+1}{k} = \binom{n}{k} + \binom{n}{k-1}$, see~\cite{Worpitzky1883}.
    \end{proof}
\end{lemma}
...
\begin{proposition} [Ordinary sums of powers in Eulerian numbers]
    ...
    \begin{mdframed}
        For non-negative integers $n,m$ and an arbitrary integer $t$
        \begin{align*}
            \KnuthRFoldSum{1}{n}{m} &= \sum_{j=0}^{m} \sum_{k=0}^{m} \left[ (-1)^j \binom{t}{j+1} + \binom{j-t+n}{j+1} \right] \eulerian{m}{k} \binom{t+k-j}{m-j}
        \end{align*}
    \end{mdframed}
\end{proposition}
